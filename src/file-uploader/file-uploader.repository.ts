...
export class FileUploaderRepository {
  constructor(
    @Inject(PROVIDERS.DATABASE) private readonly db: Kysely<Database>,
  ) {}

  async getMostActiveUsersWithAvatars(
    minAvatarsCount: number,
    minAge: number,
    maxAge: number,
  ): Promise<UserWithLatestAvatar[]> {
      ...
      .innerJoin(
        (eb) =>
          eb
            ...
            .innerJoin('user_avatars as ua', (join) =>
              join
                .onRef('ua.user_id', '=', 'u2.id')
                .on('ua.deleted_at', 'is', null),
            )
            .select(['u2.id'])
        ...
        (eb) =>
          eb
            ...
            .select([
              'ua2.user_id',
              'ua2.avatar_path',
              'ua2.created_at as avatar_created_at',
            ])
